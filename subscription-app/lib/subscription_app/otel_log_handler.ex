defmodule SubscriptionApp.OtelLogHandler do
  @moduledoc """
  ...
  original Flask service's OTLP log pipeline (opentelemetry-instrumentation-
  logging) is reproduced here with a small hand-rolled exporter. Records include
  `service.name` and, when a span is active, the trace/span id for correlation.
    ...
    case meta[:mfa] do
      {mod, _fun, _arity} ->
        ...
        String.starts_with?(name, "httpc") or
          ...
          String.starts_with?(name, "Elixir.SubscriptionApp.OtelLogHandler") or
